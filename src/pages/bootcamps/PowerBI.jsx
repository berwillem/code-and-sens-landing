...
import dashboardImg from '../../assets/powerbi/dashboard.avif';
import pbiLogo from '../../assets/powerbi/powerbi.svg';
import data1Img from '../../assets/powerbi/data1.png';
import data2Img from '../../assets/powerbi/data2.png';
  ...
  return (
    <div className="flex flex-col font-sans overflow-x-hidden" style={pageStyle} ref={containerRef}>
      ...
      <style>{`
        ...
        .pbi-grid-bg {
          background-image:
            linear-gradient(rgba(140,0,255,0.04) 1px, transparent 1px),
            linear-gradient(90deg, rgba(140,0,255,0.04) 1px, transparent 1px);
      ...
      <section className="relative w-full px-6 lg:px-16 pt-10 pb-24 lg:py-8 flex flex-col lg:flex-row items-center justify-between gap-12 min-h-[calc(100vh-5rem)] overflow-hidden">
        ...
        {/* Bg grid */}
        <div className="absolute inset-0 pbi-grid-bg pointer-events-none"></div>
        {/* Purple glow orbs */}
        ...
        <div className="flex-1 space-y-6 relative z-10 max-w-xl" style={{ textAlign: isRtl ? 'right' : 'left' }}>
          ...
          >
            <img src={pbiLogo} alt="Power BI" className="w-4 h-4" />
            {t('pbiTag', 'Programme 12 Semaines — Data & Power BI')}
          </div>

          <h1 className="hero-element text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight font-heading">
            {t('pbiHeroTitle1')}{' '}
            <span style={{ color: primaryColor }}>{t('pbiHeroHighlight')}</span>
            <br />{t('pbiHeroTitle2')}
          </h1>
          ...
          <div className="hero-element flex flex-wrap gap-6 pt-2">
            <div>
              ...
              <div className="text-xs text-slate-500">{t('pbiStatLabel1', 'Semaines intensives')}</div>
            </div>
            <div className="w-px bg-slate-200 self-stretch"></div>
            <div>
              <div className="text-2xl font-bold font-heading text-slate-800">DAX</div>
              <div className="text-xs text-slate-500">{t('pbiStatLabel2', 'Power Query & Modélisation')}</div>
            </div>
            <div className="w-px bg-slate-200 self-stretch"></div>
            <div>
        ...
        <div className="hero-element flex-1 w-full max-w-sm sm:max-w-md md:max-w-xl mx-auto relative group mt-4 lg:mt-0 px-4 sm:px-0 z-10">
          ...
          {/* Main dashboard image — floating animation */}
          <div className="relative dashboard-float">
            <img
              src={dashboardImg}
              alt="Power BI Dashboard"
              className="w-full h-auto rounded-2xl shadow-2xl border border-white/40 relative z-10"
              style={{ boxShadow: `0 30px 80px ${primaryColor}25, 0 0 0 1px ${primaryColor}15` }}
            />

            {/* Floating Power BI badge top-right */}
            <div
              ref={el => el && floatingBadgesRef.current.push(el)}
              className="pbi-badge-pill absolute -top-4 -right-2 sm:-right-8 z-20 flex items-center gap-2 px-3 py-2 rounded-2xl bg-white/90 border border-white shadow-xl"
            >
              <img src={pbiLogo} alt="Power BI" className="w-6 h-6" />
              <span className="text-xs font-bold text-slate-800 font-mono">Power BI</span>
            </div>

            {/* Floating data icon bottom-left */}
            <div
              ref={el => el && floatingBadgesRef.current.push(el)}
              className="pbi-badge-pill absolute -bottom-4 -left-2 sm:-left-8 z-20 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/90 border border-white shadow-xl"
            >
              <img src={data1Img} alt="Data Analysis" className="w-6 h-6" />
              <span className="text-xs font-bold text-slate-800 font-mono">Data Viz</span>
            </div>

            {/* Floating data2 icon top-left (small) */}
            <div
              ref={el => el && floatingBadgesRef.current.push(el)}
              className="pbi-badge-pill absolute top-1/3 -left-3 sm:-left-10 z-20 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/90 border border-white shadow-xl"
            >
              <img src={data2Img} alt="Analytics" className="w-6 h-6" />
              <span className="text-xs font-bold text-slate-800 font-mono">Analytics</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 py-24" ref={featuresRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-4xl lg:text-5xl font-bold font-heading">{t('pbiFeaturesTitle')}</h2>
            <p className="text-lg text-slate-500 max-w-2xl mx-auto">{t('pbiFeaturesSubtitle')}</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[260px]">

            {/* Card 1: Wide — Power BI Mastery */}
            <div className="bento-card md:col-span-2 bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col justify-between group overflow-hidden relative transition-all hover:shadow-lg">
              ...
              <div className="relative z-10 w-16 h-16 mb-4">
                <img src={pbiLogo} alt="Power BI" className="w-full h-full object-contain" />
              </div>
              <div className="relative z-10">
                <span className="text-xs font-bold tracking-widest uppercase mb-1 block" style={{ color: primaryColor }}>{t('pbiCard1Subtitle')}</span>
                <h3 className="text-2xl font-bold font-heading mb-2 text-slate-800">{t('pbiCard1Title')}</h3>
                <p className="text-slate-500 max-w-md">{t('pbiCard1Desc')}</p>
              </div>
            </div>

            {/* Card 2: Tall dark — Power Query & DAX */}
            <div className="bento-card md:col-span-1 md:row-span-2 bg-slate-950 rounded-3xl p-8 shadow-xl flex flex-col group overflow-hidden relative">
              ...
              <div className="relative z-10 mt-8">
                <h3 className="text-2xl font-bold font-heading mb-3 text-white">{t('pbiCard2Title')}</h3>
                <p className="text-white/70 leading-relaxed">{t('pbiCard2Desc')}</p>
                <div className="flex flex-wrap gap-1.5 mt-5">
                  {['Power Query', 'DAX', 'M Language', 'Star Schema'].map(tool => (
            ...
            {/* Card 3: Small — Visualisation & Storytelling */}
            <div className="bento-card md:col-span-1 bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col justify-center group relative overflow-hidden transition-all hover:shadow-lg">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center p-1" style={{ backgroundColor: `${primaryColor}10` }}>
                  <img src={data1Img} alt="Data viz" className="w-full h-full object-contain" />
                </div>
                <h3 className="text-xl font-bold font-heading text-slate-800">{t('pbiCard3Title')}</h3>
              </div>
              <p className="text-slate-500 text-sm">{t('pbiCard3Desc')}</p>
            </div>

            {/* Card 4: Small primary — Real Impact */}
            <div className="bento-card md:col-span-1 rounded-3xl p-8 shadow-lg flex flex-col justify-center text-white relative overflow-hidden" style={{ backgroundColor: primaryColor }}>
              ...
              <div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center mb-4 relative z-10 p-2">
                <img src={data2Img} alt="Growth" className="w-full h-full object-contain" />
              </div>
              <div className="relative z-10">
                <h3 className="text-xl font-bold font-heading mb-2">{t('pbiCard4Title')}</h3>
                <p className="text-white/80 text-sm leading-relaxed">{t('pbiCard4Desc')}</p>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-4xl lg:text-5xl font-bold font-heading text-white">{t('pbiTestimonialsTitle')}</h2>
      ...
      <section id="curriculum" className="py-24 bg-white" ref={curriculumRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="curriculum-header text-center mb-16 space-y-4">
            <h2 className="text-4xl lg:text-5xl font-bold font-heading">{t('pbiCurriculumTitle')}</h2>
            <p className="text-lg text-slate-500 max-w-2xl mx-auto">{t('pbiCurriculumSubtitle')}</p>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {(t('pbiCurriculumModules', { returnObjects: true }) || []).map((mod, i) => (
              <div
                key={i}
          ...
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-slate-100">
            ...
            >
              ...
              <p className="text-white/80 text-base leading-relaxed mb-8">
                {t('pbiContactSubtitle')}
              </p>
              <ul className="space-y-3">
                {[t('contactCheck1'), t('contactCheck2'), t('pbiContactCheck3')].map((item, i) => (
            ...
            <div className="md:w-3/5 p-10 md:p-14">
              ...
              <form className="space-y-5" onSubmit={async (e) => {
                ...
                <div className="space-y-1.5">
                  <label htmlFor="pbi-name" className="text-sm font-semibold text-slate-600">{t('formName')}</label>
                  <input type="text" id="pbi-name" className="input w-full" placeholder="Ahmed Benali" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="pbi-email" className="text-sm font-semibold text-slate-600">{t('formEmail')}</label>
                  <input type="email" id="pbi-email" className="input w-full" placeholder="ahmed@example.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="pbi-phone" className="text-sm font-semibold text-slate-600">{t('formPhone')}</label>
                  <div className="flex" dir="ltr">
                    ...
                    <input type="tel" id="pbi-phone" className="input rounded-l-none w-full" placeholder="555 123 456" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} required />
                  </div>
                </div>
                <button
                  type="submit"
