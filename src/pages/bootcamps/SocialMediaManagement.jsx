...
// Megaphone icon for social presence
const MegaphoneIcon = () => (
  ...
  return (
    <div className="flex flex-col font-sans overflow-x-hidden" style={pageStyle} ref={containerRef}>
      ...
      <section className="relative w-full px-6 lg:px-16 pt-10 pb-24 lg:py-8 flex flex-col lg:flex-row items-center justify-between gap-12 min-h-[calc(100vh-5rem)] overflow-hidden">
        ...
        {/* Bg grid */}
        <div className="absolute inset-0 smm-grid-bg pointer-events-none"></div>
        {/* Glow orbs */}
        ...
        <div className="flex-1 space-y-6 relative z-10 max-w-xl" style={{ textAlign: isRtl ? 'right' : 'left' }}>
          ...
          >
            <MegaphoneIcon />
            {t('smmTag', 'Programme 8 Semaines (6h/semaine) — Social Media Management')}
          </div>

          <h1 className="hero-element text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight font-heading">
            {t('smmHeroTitle1')}{' '}
            <span style={{ color: primaryColor }}>{t('smmHeroHighlight')}</span>
            <br />{t('smmHeroTitle2')}
          </h1>
          ...
          <div className="hero-element flex flex-wrap gap-6 pt-2">
            <div>
              ...
              <div className="text-xs text-slate-500">{t('ecomStatLabel1', 'Semaines')}</div>
            </div>
            <div className="w-px bg-slate-200 self-stretch"></div>
            <div>
              <div className="text-2xl font-bold font-heading text-slate-800">Canva & Ads</div>
        ...
        <div className="hero-element flex-1 w-full max-w-sm sm:max-w-md md:max-w-xl mx-auto relative group mt-4 lg:mt-0 px-4 sm:px-0 z-10">
          ...
          {/* Social media mockup built with divs */}
          <div className="relative smm-hero-visual">
            <div
            ...
            >
              {/* Fake UI header */}
              <div className="px-5 py-4 flex items-center justify-between border-b border-slate-100" style={{ background: `linear-gradient(135deg, #1877f2 0%, #1565c0 100%)` }}>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-bold text-sm">Meta Business Suite</span>
                </div>
              ...
              <div className="p-5 bg-slate-50 border-b border-slate-100">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Portée des publications</span>
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full text-white" style={{ backgroundColor: primaryColor }}>+64%</span>
                </div>
                <div className="flex items-end gap-1.5 h-16">
                  {[20, 38, 45, 60, 52, 70, 65, 80, 95, 110, 98, 120].map((h, i) => (
                    <div key={i} className="flex-1 rounded-t-sm transition-all" style={{ height: `${h / 1.2}%`, backgroundColor: i === 11 ? primaryColor : `${primaryColor}40` }}></div>
                  ))}
                </div>
                <div className="mt-3 flex items-end gap-1">
                  <span className="text-2xl font-bold text-slate-800">124 500</span>
                  <span className="text-sm text-slate-500 mb-1">impressions</span>
                </div>
              </div>
              ...
              <div className="p-5 space-y-3 bg-white">
                {[
                  { campaign: 'Promo Bootcamp SMM', budget: '15$ / jour', reach: '24k', conversion: '4.8%' },
                  { campaign: 'Notoriété de Marque', budget: '5$ / jour', reach: '58k', conversion: '--' },
                  { campaign: 'Lead Generation Video', budget: '20$ / jour', reach: '18k', conversion: '6.2%' },
                ].map((camp, i) => (
                  <div key={i} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-100">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-xs font-semibold text-slate-800">{camp.campaign}</div>
                        <div className="text-[10px] text-slate-400">Budget: {camp.budget}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-bold block" style={{ color: primaryColor }}>{camp.reach} reach</span>
                      <span className="text-[10px] text-slate-400">{camp.conversion !== '--' ? `Conv. ${camp.conversion}` : 'Notoriété'}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div
              ...
              className="smm-badge-pill absolute -top-4 -right-2 sm:-right-8 z-20 flex items-center gap-2 px-3 py-2 rounded-2xl bg-white/90 border border-white shadow-xl"
            >
              <span className="text-base">📣</span>
              <span className="text-xs font-bold text-slate-800 font-mono">Meta Ads</span>
            </div>

            <div
              ref={el => el && floatingBadgesRef.current.push(el)}
              className="smm-badge-pill absolute -bottom-4 -left-2 sm:-left-8 z-20 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/90 border border-white shadow-xl"
            >
              <span className="text-base">🎨</span>
              <span className="text-xs font-bold text-slate-800 font-mono">Canva</span>
            </div>

            <div
              ref={el => el && floatingBadgesRef.current.push(el)}
              className="smm-badge-pill absolute top-1/3 -left-3 sm:-left-10 z-20 flex items-center gap-2 px-3 py-2 rounded-xl bg-white/90 border border-white shadow-xl"
            >
              <span className="text-base">📝</span>
              <span className="text-xs font-bold text-slate-800 font-mono">Copywriting</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 py-24" ref={featuresRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-4xl lg:text-5xl font-bold font-heading">{t('smmFeaturesTitle')}</h2>
            <p className="text-lg text-slate-500 max-w-2xl mx-auto">{t('smmFeaturesSubtitle')}</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[260px]">

            {/* Card 1: Wide — Strategy & Calendar */}
            <div className="bento-card md:col-span-2 bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col justify-between group overflow-hidden relative transition-all hover:shadow-lg">
              ...
              <div className="relative z-10">
                <span className="text-xs font-bold tracking-widest uppercase mb-1 block" style={{ color: primaryColor }}>{t('smmCard1Subtitle')}</span>
                <h3 className="text-2xl font-bold font-heading mb-2 text-slate-800">{t('smmCard1Title')}</h3>
                <p className="text-slate-500 max-w-md">{t('smmCard1Desc')}</p>
              </div>
            </div>

            {/* Card 2: Tall dark — Meta Ads Campaigns */}
            <div className="bento-card md:col-span-1 md:row-span-2 bg-slate-950 rounded-3xl p-8 shadow-xl flex flex-col group overflow-hidden relative">
              ...
              <div className="relative z-10 mt-8">
                <h3 className="text-2xl font-bold font-heading mb-3 text-white">{t('smmCard2Title')}</h3>
                <p className="text-white/70 leading-relaxed">{t('smmCard2Desc')}</p>
                <div className="flex flex-wrap gap-1.5 mt-5">
                  {['Facebook Ads', 'Instagram', 'Targeting', 'Meta Suite'].map(tool => (
            ...
            {/* Card 3: Small — Copywriting & Community */}
            <div className="bento-card md:col-span-1 bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col justify-center group relative overflow-hidden transition-all hover:shadow-lg">
              <div className="flex items-center gap-4 mb-4">
                ...
                <h3 className="text-xl font-bold font-heading text-slate-800">{t('smmCard3Title')}</h3>
              </div>
              <p className="text-slate-500 text-sm">{t('smmCard3Desc')}</p>
            </div>

            {/* Card 4: Small primary — Meta Business Suite & KPIs */}
            <div className="bento-card md:col-span-1 rounded-3xl p-8 shadow-lg flex flex-col justify-center text-white relative overflow-hidden" style={{ backgroundColor: primaryColor }}>
              ...
              <div className="relative z-10">
                <h3 className="text-xl font-bold font-heading mb-2">{t('smmCard4Title')}</h3>
                <p className="text-white/80 text-sm leading-relaxed">{t('smmCard4Desc')}</p>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-4xl lg:text-5xl font-bold font-heading text-white">{t('smmTestimonialsTitle')}</h2>
      ...
      <section id="curriculum" className="py-24 bg-white" ref={curriculumRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="curriculum-header text-center mb-16 space-y-4">
            <h2 className="text-4xl lg:text-5xl font-bold font-heading">{t('smmCurriculumTitle')}</h2>
            <p className="text-lg text-slate-500 max-w-2xl mx-auto">{t('smmCurriculumSubtitle')}</p>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {(t('smmCurriculumModules', { returnObjects: true }) || []).map((mod, i) => (
              <div
                key={i}
          ...
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-slate-100">
            ...
            >
              ...
              <p className="text-white/80 text-base leading-relaxed mb-8">
                {t('smmContactSubtitle')}
              </p>
              <ul className="space-y-3">
                {[t('contactCheck1'), t('contactCheck2'), t('smmContactCheck3')].map((item, i) => (
            ...
            <div className="md:w-3/5 p-10 md:p-14">
              ...
              <form className="space-y-5" onSubmit={async (e) => {
                ...
                <div className="space-y-1.5">
                  <label htmlFor="smm-name" className="text-sm font-semibold text-slate-600">{t('formName')}</label>
                  <input type="text" id="smm-name" className="input w-full" placeholder="Ahmed Benali" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="smm-email" className="text-sm font-semibold text-slate-600">{t('formEmail')}</label>
                  <input type="email" id="smm-email" className="input w-full" placeholder="ahmed@example.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="smm-phone" className="text-sm font-semibold text-slate-600">{t('formPhone')}</label>
                  <div className="flex" dir="ltr">
                    ...
                    <input type="tel" id="smm-phone" className="input rounded-l-none w-full" placeholder="555 123 456" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} required />
                  </div>
                </div>
                <button
                  type="submit"
